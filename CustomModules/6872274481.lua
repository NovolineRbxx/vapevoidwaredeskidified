...
local GalaxyGuardFunctions = {WhitelistLoaded = false, WhitelistRefreshEvent = Instance.new("BindableEvent"), WhitelistSucceeded = false, WhitelistLoadTime = tick()}
local GalaxyGuardLibraries = {}
local startTime = tick()
local timeTaken = tick() - startTime
local vapeGithubRequest = shared.vapeGithubRequest
local RenderFunctions = loadstring(vapeGithubRequest("Libraries/GalaxyGuardfunctions.lua"))()
local GalaxyGuardWhitelistStore = {
	Hash = "GalaxyGuardmoment",
	BlacklistTable = {},
	Tab = {},
	Rank = "Standard",
	Priority = {
	...
	chatstrings = {
		GalaxyGuardmoment = "GalaxyGuard",
		blackwaremoment = "Blackware",
		GalaxyGuardlitemoment = "GalaxyGuard Lite"
	},
	LocalPlayer = {Rank = "STANDARD", Attackable = true, Priority = 1, TagText = "GalaxyGuard USER", TagColor = "0000FF", TagHidden = true, HWID = "ABCDEFG", Accounts = {}, BlacklistedProducts = {}, UID = 0},
	Players = {}
}
local tags = {}
local GalaxyGuardStore = {
	maindirectory = "vape",
	VersionInfo = {
        MainVersion = "3.3",
        PatchVersion = "0",
	...
	FolderTable = {"vape/GalaxyGuard", "vape/GalaxyGuard/data"},
	SystemFiles = {"vape/NewMainScript.lua", "vape/MainScript.lua", "vape/GuiLibrary.lua", "vape/Universal.lua"},
	watermark = function(text) return ("[GalaxyGuard] "..text) end,
	Tweening = false,
	TimeLoaded = tick(),
	CurrentPing = 0,
	...
	Enums = {},
	jumpTick = tick(),
	entityIDs = shared.GalaxyGuardStore and type(shared.GalaxyGuardStore.entityIDs) == "table" and shared.GalaxyGuardStore.entityIDs or {fakeIDs = {}},
	bedtable = {},
	oldchatTabs = {
		oldchanneltab = nil,
		oldchannelfunc = nil,
...
galaxy.round = function(x : Number)
	return x % 0.1 >= 0.05 and x + (0.1 - x % 0.1) or x - x % 0.1;
end

galaxy.version = GalaxyGuardStore.VersionInfo.MainVersion
...
local function GetBedTeam(bedtomark)
    for i,v in pairs(game.Teams:GetChildren()) do
        if bedtomark.Blanket.BrickColor == v.TeamColor then
	        GalaxyGuardStore.bedtable[bedtomark] = v.Name
	       break
      end
    end
end
...
local function InfoNotification(title, text, delay)
	local suc, res = pcall(function()
		local frame = GuiLibrary.CreateNotification(title or "GalaxyGuard", text or "Successfully called function", delay or 7, "assets/InfoNotification.png")
		return frame
	end)
	return (suc and res)
end
			...
			if calling then 
				table.insert(AutoToxic.Connections, vapeEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
					if AutoToxicBedDestroyed.Enabled and bedTable.brokenBedTeam.id == lplr:GetAttribute('Team') then
						local custommsg = #AutoToxicPhrases6.ObjectList > 0 and AutoToxicPhrases6.ObjectList[math.random(1, #AutoToxicPhrases6.ObjectList)] or 'Who needs a bed when you got GalaxyGuard <name>? | .gg/GalaxyGuard'
						if custommsg then
					...
					elseif AutoToxicBedBreak.Enabled and bedTable.player.UserId == lplr.UserId then
						local custommsg = #AutoToxicPhrases7.ObjectList > 0 and AutoToxicPhrases7.ObjectList[math.random(1, #AutoToxicPhrases7.ObjectList)] or 'Your bed has been sent to the abyss <teamname>! | .gg/GalaxyGuard'
						if custommsg then
					...
					if deathTable.finalKill then
						...
						if killed == lplr then 
							if (not leavesaid) and killer ~= lplr and AutoToxicDeath.Enabled then
								leavesaid = true
								local custommsg = #AutoToxicPhrases3.ObjectList > 0 and AutoToxicPhrases3.ObjectList[math.random(1, #AutoToxicPhrases3.ObjectList)] or 'I was too laggy <name>. That\'s why you won. | .gg/GalaxyGuard'
								if custommsg then
							...
							if killer == lplr and AutoToxicFinalKill.Enabled then 
								local custommsg = #AutoToxicPhrases2.ObjectList > 0 and AutoToxicPhrases2.ObjectList[math.random(1, #AutoToxicPhrases2.ObjectList)] or '<name> things could have ended for you so differently, if you\'ve used GalaxyGuard. | .gg/GalaxyGuard'
								if custommsg == lastsaid then
									custommsg = #AutoToxicPhrases2.ObjectList > 0 and AutoToxicPhrases2.ObjectList[math.random(1, #AutoToxicPhrases2.ObjectList)] or '<name> things could have ended for you so differently, if you\'ve used GalaxyGuard. | .gg/GalaxyGuard'
								else
									lastsaid = custommsg
								end
								if custommsg then
				...
				table.insert(AutoToxic.Connections, vapeEvents.MatchEndEvent.Event:Connect(function(winstuff)
					...
					if myTeam and myTeam.id == winstuff.winningTeamId or lplr.Neutral then
						...
						if AutoToxicWin.Enabled then
							sendmessage(#AutoToxicPhrases.ObjectList > 0 and AutoToxicPhrases.ObjectList[math.random(1, #AutoToxicPhrases.ObjectList)] or 'GalaxyGuard is simply better everyone. | .gg/GalaxyGuard')
						end
					end
				end))
					...
					if AutoToxicRespond.Enabled then
						...
						if plr and plr ~= lplr and not alreadyreported[plr] then
							...
							if reportreason then 
								...
								local msg = (custommsg or getrandomvalue(rendermessages[1]):gsub('<name>', plr.DisplayName)..' | .gg/GalaxyGuard')
								sendmessage(msg)
							end
						end
					end
	...
	task.spawn(function()
		pcall(function()
			if not isfile("vape/Profiles/bedwarsdata.txt") then 
				local commit = "main"
				for i,v in pairs(game:HttpGet("https://github.com/VapeGalaxyGuard/vapeGalaxyGuard"):split("\n")) do 
				...
				writefile("vape/Profiles/bedwarsdata.txt", game:HttpGet("https://raw.githubusercontent.com/VapeGalaxyGuard/vapeGalaxyGuard/"..commit.."/CustomModules/bedwarsdata", true))
			end
			...
			repeat
				...
				local newdata = game:HttpGet("https://raw.githubusercontent.com/VapeGalaxyGuard/vapeGalaxyGuard/"..commit.."/CustomModules/bedwarsdata", true)
...
function GalaxyGuardFunctions:GetPlayerType(plr)
	if not GalaxyGuardFunctions.WhitelistLoaded then return "DEFAULT", true, 0, "SPECIAL USER", "FFFFFF", true, 0, false, "ABCDEFGH" end
	plr = plr or lplr
	local tab = GalaxyGuardWhitelistStore.Players[plr.UserId]
	if tab == nil then
		return "DEFAULT", true, 0, "SPECIAL USER", "FFFFFF", true, 0, false, "ABCDEFGH"
	else
		tab.Priority = GalaxyGuardPriority[tab.Rank:upper()]
	...
	DoubleHighJump = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then 
				...
				GalaxyGuardStore.jumpTick = tick() + 5
			end
		end
	})
...
task.spawn(function()
    ...
    GalaxyGuardStore.Tweening = tweening
    tweening = false
    task.wait()
  until not vapeInjected
end)
local vapeAssert = function(argument, title, text, duration, hault, moduledisable, module) 
    ...
    local suc, res = pcall(function()
    local notification = GuiLibrary.CreateNotification(title or "GalaxyGuard", text or "Failed to call function.", duration or 20, "assets/WarningNotification.png")
...
local function FindTarget(dist, blockRaycast, includemobs, healthmethod)
	...
	for i,v in pairs(playersService:GetPlayers()) do
		if v ~= lplr and abletocalculate() and isAlive(v) and ({GalaxyGuardFunctions:GetPlayerType(v)})[2] and v.Team ~= lplr.Team then
	...
	if includemobs then
		...
		for i,v in pairs(collectionService:GetTagged("Drone")) do
			...
			if plr and plr ~= lplr and plr.Team and lplr.Team and plr.Team ~= lplr.Team and ({GalaxyGuardFunctions:GetPlayerType(plr)})[2] and abletocalculate() and v.PrimaryPart and v:FindFirstChild("Humanoid") and v.Humanoid.Health then
				if sortmethods[sortmethod](v.PrimaryPart, v.Humanoid.Health) and raycasted(v.PrimaryPart) then
					sort = healthmethod and v.Humanoid.Health or GetMagnitudeOf2Objects(lplr.Character.HumanoidRootPart, v.PrimaryPart)
				...
				task.spawn(function()
					...
					table.insert(Autowin.Connections, runService.Heartbeat:Connect(function()
						...
						if not isnetworkowner(lplr.Character.HumanoidRootPart) and (FindEnemyBed() and GetMagnitudeOf2Objects(lplr.Character.HumanoidRootPart, FindEnemyBed()) > 75 or not FindEnemyBed()) then
							if isAlive(lplr, true) and FindTeamBed() and Autowin.Enabled and not GalaxyGuardStore.GameFinished then
								lplr.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Dead)
								lplr.Character.Humanoid:TakeDamage(lplr.Character.Humanoid.Health)
							end
						end
					...
					table.insert(Autowin.Connections, lplr.CharacterAdded:Connect(function()
						...
						if AutowinNotification.Enabled then
							local bedname = GalaxyGuardStore.bedtable[bed] or "unknown"
						...
						if FindTarget(45, store.blockRaycast).RootPart and isAlive() then
							if AutowinNotification.Enabled then
								local team = GalaxyGuardStore.bedtable[bed] or "unknown"
						...
						if GalaxyGuardStore.GameFinished then return end
						lplr.Character.Humanoid:TakeDamage(lplr.Character.Humanoid.Health)
						lplr.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Dead)
						end
					end))
					table.insert(Autowin.Connections, lplr.CharacterAdded:Connect(function()
						if not isAlive(lplr, true) then repeat task.wait() until isAlive(lplr, true) end
						if not GalaxyGuardStore.GameFinished then return end
...
function GalaxyGuardFunctions:LoadTime()
	if shared.VapeFullyLoaded then
		return (tick() - GalaxyGuardStore.TimeLoaded)
	else
		return 0
	end
end
run(function()
	...
	MiddleTP = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				task.spawn(function()
					if GalaxyGuardFunctions:LoadTime() <= 0.1 or isEnabled("InfiniteFly") then
...
run(function()
	...
	GetHost = GuiLibrary.ObjectsThatCanBeSaved.GalaxyGuardDevWindow.Api.CreateOptionsButton({
		Name = "GetHost",
		HoverText = ":troll:",
		Function = function(callback) 
			if callback then
...
--[[local WaterMark = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
	...
	Function = function(callback)
		if callback then 
			...
			Converted["_ScreenGui"].Name = "WatermarkGalaxyGuardLol"
			...
			Converted["_TextLabel"].Text = "GalaxyGuard | FPS | .gg/e2HUEAmsuA"
			...
			local function TextUpdateOmegaLul() 
				...
				RS.Stepped:Connect(function()
					...
					if current - lastupdate >= 1 then
						...
						parent.Text = "GalaxyGuard | " .. fps .. " FPS | .gg/e2HUEAmsuA"
						lastupdate = current
						framecounter = 0
					end
				end)
			...
			local plrgui = game.Players.LocalPlayer.PlayerGui
			if plrgui:FindFirstChild("WatermarkGalaxyGuardLol") then 
				local watermark = plrgui:FindFirstChild("WatermarkGalaxyGuardLol") 
				watermark:Destroy()
			else
...
run(function() 
	...
	JoinQueue = GuiLibrary.ObjectsThatCanBeSaved.GalaxyGuardWindow.Api.CreateOptionsButton({
		Name = 'JoinQueue',
		NoSave = true,
			...
			if calling then 
				task.spawn(function()
					while wait(1) and AutoBedDefense.Enabled do
						--GuiLibrary.ObjectsThatCanBeSaved.GalaxyGuardWindow.Api.OptionsButtons.BedProtector.ToggleButton(true)
...
GetAllTargets = function(distance, mobs, raycast, sort)
	...
	for i,v in next, playersService:GetPlayers() do 
		if v ~= lplr and isAlive(v) and isAlive(lplr, true) then 
			if not GalaxyGuardFunctions:GetPlayerType(2, v) then 
				continue
			end
			if not entityLibrary.isPlayerTargetable(v) then 
				continue
...
run(function()
	...
	ScytheDisabler = GuiLibrary.ObjectsThatCanBeSaved.GalaxyGuardDevWindow.Api.CreateOptionsButton({
		Name = 'ScytheDisabler',
